...
import static com.example.projectsapp.projectsapp.util.Util.TIME_PATTERN;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<?> handleApiRequestException(ProjectAssignException ex) {
        log.error(String.format("status %s. User Id/s not found and can't be added to project", HttpStatus.NOT_FOUND));
        ApiError error = new ApiError(
                LocalDateTime.now().format(DateTimeFormatter.ofPattern(TIME_PATTERN)),
                HttpStatus.NOT_FOUND.value(),
                HttpStatus.NOT_FOUND.getReasonPhrase(),
